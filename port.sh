...
echo "ВАЖНО: Не закрывайте это соединение, пока не проверите, что можете подключиться через новый порт!"
echo "Для подключения используйте: ssh -p $new_port user@your_vps_ip"